...
out=$(curl -X POST -d '{"data": "Something cool"}' -H "Content-Type: application/json" -H "X-TOKEN: $token" -H "X-SERVICE: cool_thing" -H "X-TARGET-SERVICE: cool_thing" http://0.0.0.0:8000/subscription/forward)
echo $out
echo -e
